...
from models import db, User, Question, TestResult, WeakQuestion, UserStats
...
def start_test():
    ...
    if mode == 'all':
        ...
        timer_min = 10 # Fixed 10 mins for 20 Qs
...
def weak_questions():
    ...
    weaks = WeakQuestion.query.filter_by(user_id=user.id).all()
    result = []
    for w in weaks:
...
def add_question():
    ...
    last_test = Question.query.filter_by(category=category, subcategory=subcategory)\
        .order_by(Question.test_number.desc()).first()
        
    test_number = 1
    if last_test:
        count_in_last = Question.query.filter_by(
            category=category, subcategory=subcategory, test_number=last_test.test_number
        ).count()
        if count_in_last < 20:
            test_number = last_test.test_number
        else:
            test_number = last_test.test_number + 1
            
    q = Question(
        category=category, subcategory=subcategory, test_number=test_number,
...
def import_questions():
    ...
    if last_test:
        current_test_num = last_test.test_number
        count_in_current = Question.query.filter_by(
...
<section id="dashboardScreen" class="screen">
  <div class="container">
    ...
    <div class="page-head" style="margin-top:24px">
      <div><p class="eyebrow">Actions</p><h2 class="page-title">What would you like to do?</h2></div>
    </div>
    ...
    const enterApp = async () => {
      ...
      try {
        const res = await fetch('/api/login', {
          ...
          body: JSON.stringify({ username: val, user_id: state.user_id })
        });
        ...
        nav('dashboard');
      } catch (e) { toast('Login failed','error'); } finally { showLoading(false); }
    };
    startBtn.addEventListener('click', enterApp);
    nameInput.addEventListener('keypress', e => { if (e.key === 'Enter') { e.preventDefault(); enterApp(); } });
  }
  if (state.user_id) { document.getElementById('userChip').hidden = false; document.getElementById('userName').textContent = state.username; renderScreen(window.location.hash.replace('#','') || 'dashboard'); }
});
...
async function renderDashboard() {
  ...
  grid.innerHTML = `
    ...
    <button class="action-card stagger-item" style="animation-delay:0.35s" onclick="nav('manage')"><div class="ac-icon g2">➕</div><h3>Add Question</h3><p>Contribute to bank.</p><span class="ac-arrow">→</span></button>
  ...
  // No specific recent API, let's use a dummy if needed or skip
...
function renderTestQuestion() {
  ...
  // Instant render, no slide animation
  card.innerHTML = `
  ...
  card.querySelectorAll('.opt').forEach(b => b.addEventListener('click', () => {
    ...
    renderTestQuestion(); // Re-render instantly
  }));
